...
# ── Configurable STT language (empty = auto-detect) ──
STT_LANGUAGE = os.environ.get("STT_LANGUAGE", "")

# ── Wake word support (optional dependency) ──
WAKE_WORD_ENABLED = os.environ.get("WAKE_WORD", "").lower() in ("1", "true", "yes")
...
def _is_tts_playing():
    """Check if TTS playback is currently running."""
    if IS_LINUX:
        r = subprocess.run(["pgrep", "-f", "mpv"], capture_output=True)
...
def _init_wake_word():
    """Lazily initialize the wake word model."""
    global _wake_word_model
    if _wake_word_model is not None or not _oww_available:
        return
    try:
        _wake_word_model = _OwwModel(wakeword_models=["hey_jarvis"])
        print("Wake word model loaded (hey_jarvis).", flush=True)
    except Exception as e:
        print(f"Wake word model failed to load: {e}", flush=True)
...
def _activate_app(name: str):
    ...
    if IS_LINUX:
        subprocess.run(["xdotool", "windowactivate", name], capture_output=True)
        return
    subprocess.run(
...
def _persistent_callback(indata, frames, time_info, status):
    ...
    # Wake word mode
    if _wake_word_active and _wake_word_model is not None and not _pressed:
        global _wake_word_triggered, _wake_word_silence_count
        audio_int16 = (indata.flatten() * 32767).astype(np.int16)
        if not _wake_word_triggered:
            _wake_word_model.predict(audio_int16)
            for score in _wake_word_model.prediction_buffer.values():
                if len(score) > 0 and score[-1] > WAKE_WORD_SCORE_THRESHOLD:
                    _wake_word_triggered = True
                    _wake_word_silence_count = 0
                    _wake_word_chunks.clear()
                    _wake_word_chunks.append(indata.copy())
                    open("/tmp/recording_active", "w").close()
                    _target_app = _get_frontmost_app()
                    if _is_tts_playing():
                        interrupt_tts()
                    print("  🎙️ Wake word detected! Recording...", flush=True)
                    break
        else:
            _wake_word_chunks.append(indata.copy())
            rms_ww = float(np.sqrt(np.mean(indata.flatten() ** 2)))
            if rms_ww < VAD_ENERGY_THRESHOLD:
                _wake_word_silence_count += 1
            else:
                _wake_word_silence_count = 0
            if _wake_word_silence_count >= VAD_SILENCE_TIMEOUT_FRAMES:
                _wake_word_triggered = False
                chunks_copy = list(_wake_word_chunks)
                _wake_word_chunks.clear()
                if len(chunks_copy) > 10:
                    ...
                    print(f"  🎙️ Wake word: Done, transcribing...", flush=True)
                else:
                    try:
                        os.unlink("/tmp/recording_active")
                    except FileNotFoundError:
...
def _transcribe_inner(chunks):
    ...
    try:
        ...
        if STT_ENGINE == "groq":
            try:
                _groq_kwargs = dict(
                    file=("audio.wav", open(tmp_path, "rb").read()),
                    model="whisper-large-v3-turbo",
                    response_format="text",
                )
                if STT_LANGUAGE:
                    _groq_kwargs["language"] = STT_LANGUAGE
                result = _groq_client.audio.transcriptions.create(**_groq_kwargs)
                text = result.strip() if isinstance(result, str) else result.text.strip()
                text = text or None
            except Exception as e:
                print(f"  (Groq failed: {e}, falling back to Apple Speech)", flush=True)
                _stt_args = [STT_TOOL, tmp_path]
                if STT_LANGUAGE:
                    _stt_args.append(STT_LANGUAGE)
                else:
                    _stt_args.append("zh-Hans")
                result = subprocess.run(
                    _stt_args,
            ...
            _whisper_kwargs = {}
            if STT_LANGUAGE:
                _whisper_kwargs["language"] = STT_LANGUAGE
            segments, _ = whisper_model.transcribe(tmp_path, **_whisper_kwargs)
...
def type_text(text):
    ...
    if IS_LINUX:
        if _target_app:
            ...
            time.sleep(0.15)
        # Try xdotool type first, fall back to clipboard paste
        try:
            subprocess.run(
        ...
        if key in (keyboard.Key.shift, keyboard.Key.shift_r):
            ...
            if recording and key == keyboard.Key.shift:
                ...
                if _vad_enabled and not _wake_word_active:
                    # VAD on → try wake word
                    if _oww_available and WAKE_WORD_ENABLED:
                        _vad_enabled = False
                        ...
                        try:
                            ...
                            pass
                        open("/tmp/wake_word_mode", "w").close()
                        mode = "👂 Wake word"
                    else:
                        # No wake word available, go to OFF
                        _vad_enabled = False
                        ...
                        try:
                            ...
                            pass
                        mode = "⌥ 手动按键"
                elif _wake_word_active:
                    # Wake word → OFF
                    _wake_word_active = False
                    _vad_enabled = False
                    try:
                        os.unlink("/tmp/wake_word_mode")
                    except FileNotFoundError:
                        pass
                    mode = "⌥ 手动按键"
                else:
...
def _alt_r_physically_held():
    ...
    try:
        ...
        return False  # Quartz unavailable (Linux), assume not held
    except Exception:
